...
% 虽然这些翻译模型在小型基准数据集上取得了很好的成绩，但是这些模型的现有实现通常是单线程的，而且对于训练来说非常耗时，这对于嵌入大规模的KG来说是不太好的。为了学习大规模知识图嵌入，我们提出了一些优化方法。我们工作的优化方法有三个方面。首先，我们设计一个统一的框架来加速TransE及其扩展。该框架将KG整体分为几个部分，并采用这些模型进行多线程训练。我们还提出了基于偏移的负采样来替代原始负采样算法。新的采样机制可以合并算术运算以加速。其次，对于某些具体的模型，我们还通过TensorFlow \ Abite2016TensorFlow}来实现它们，基于我们的框架构建一个方便的平台来运行GPU上的模型\ footnote {https://github.com/thunlp/TensorFlow-TransX}。第三，除了工具包，我们还通过这个框架提供了大型KG的预先训练的嵌入。嵌入可以直接用于其他作品。
